...
const NoteState = (props) => {
  ...
  const host = "https://cleverback.vercel.app"
  // const localStorage.getItem("clevertoken") = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VyIjp7ImlkIjoiNjQ4NGE1MGNlN2IyOTExNzNlZjU4YTI1In0sImlhdCI6MTY4NjQxNDYwNH0.M0S-kT-WNsWlwL1qmgEZk0rR4e-FzN70Tp9JfcH3ebQ"
  const [quickNoteText, setQuickNoteText] = useState({description: "Christ the Redeemer"})
  ...
  const editNote = async (id, cnt) => {
    ...
    const response = await fetch(`${host}/api/notes/updatenote/${id}`, {
      ...
      headers: {
        ...
        "auth-token": localStorage.getItem("clevertoken"),
  ...
  const getNotes = async () => {
    ...
    // console.log(json)
    // return json
    console.log("The notelist is ", json)
    setNoteList(json)
  ...
  const addQnote = async () => {
    // console.log("Adding a new note")
    //todo:api call

    const response = await fetch(`${host}/api/qnote/addquicknote`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        "auth-token": localStorage.getItem("clevertoken"),
      ...
      body:JSON.stringify({id: localStorage.getItem("cleverid")})
